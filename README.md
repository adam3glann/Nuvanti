...
Cash on Delivery remains available. Paymob hosted online checkout is available after server-side keys, integration IDs, and the signed callback are configured; see the backend README. Refunds and shipping-carrier booking/tracking integrations are not automated. Product/editorial copy beyond the slideshow and site policies are maintained in source files, so update and review them before launch. See the backend README's production checklist for external setup requirements.
