...
// are registered. Only staff-tier accounts may use the admin reset flow.
router.post('/password-reset/request', async (req, res) => {
  const { email } = resetRequest.parse(req.body);
  const { rows } = await query(`SELECT id, email FROM users WHERE email = $1 AND is_active = true AND role = ANY($2)`, [email, STAFF_ROLES]);
  const user = rows[0];
  if (user) {
...
router.post('/password-reset/confirm', async (req, res) => {
  ...
  const hash = crypto.createHash('sha256').update(token).digest('hex');
  const { rows } = await query(`SELECT pr.id, pr.user_id, u.email FROM password_reset_tokens pr JOIN users u ON u.id = pr.user_id WHERE pr.token_hash = $1 AND pr.used_at IS NULL AND pr.expires_at > NOW() AND u.is_active = true AND u.role = ANY($2) FOR UPDATE`, [hash, STAFF_ROLES]);
  if (!rows[0]) return res.status(400).json({ error: 'This reset link is invalid or has expired. Request a new one.' });
  const passwordHash = await bcrypt.hash(password, 12);
  await query('UPDATE users SET password_hash = $1, failed_login_count = 0, locked_until = NULL WHERE id = $2', [passwordHash, rows[0].user_id]);
  await query('UPDATE password_reset_tokens SET used_at = NOW() WHERE user_id = $1 AND used_at IS NULL', [rows[0].user_id]);
  await logAudit({ req, actor: { id: rows[0].user_id, email: rows[0].email }, action: 'auth.password_reset_completed', targetType: 'user', targetId: rows[0].user_id });
  res.status(204).end();
});
