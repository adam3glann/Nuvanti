...
3. Set the private `DATABASE_URL`, first-admin email, and a unique first-admin password (at least 12 characters) in Render. For email, verify `nuvanti.com` in Resend, add the DNS records it gives you in Cloudflare, then set `RESEND_API_KEY` and `MAIL_FROM` (for example `Nuvanti <orders@nuvanti.com>`) in Render. This uses HTTPS and works with Render's free service, which blocks SMTP ports. Render generates `JWT_SECRET` for the service. On its first start, the service applies the PostgreSQL migrations, inserts the starter catalog once, and creates the first super-admin. The bootstrap password is never displayed; remove the bootstrap password variable after the first successful deploy.
